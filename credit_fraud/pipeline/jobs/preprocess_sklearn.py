...
if __name__ == "__main__":
    ...
    time_train, time_validation, X_train, X_validation, y_train, y_validation = train_test_split(
    ...
    X_validation_scaled = pd.DataFrame(ct.transform(X_validation), columns=X_validation.columns)
    ...
    df_validation = pd.concat([y_validation.reset_index(drop=True), X_validation_scaled], axis=1)
    ...
    df_validation.to_parquet(f"{local_dir}/validation.parquet/data.parquet", index=False)
