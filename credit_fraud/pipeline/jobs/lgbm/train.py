...
from sagemaker_jumpstart_prepack_script_utilities.prepack_inference import (
    copy_inference_code,
)
...
def _parse_args():
    ...
    parser.add_argument(
        "--train-alt", type=str, default=os.environ.get("SM_CHANNEL_TRAIN")
    )
    parser.add_argument(
        "--validation", type=str, default=os.environ.get("SM_CHANNEL_VALIDATION")
    )
    parser.add_argument(
        "--input-data-config", type=str, default=os.environ.get("SM_INPUT_DATA_CONFIG")
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pretrained-model", type=str, default=os.environ.get("SM_CHANNEL_MODEL")
    )
    parser.add_argument(
        "--num_boost_round", type=int, default=constants.DEFAULT_NUM_BOOST_ROUND
    )
    parser.add_argument(
        "--early_stopping_rounds",
        type=int,
        default=constants.DEFAULT_EARLY_STOPPING_ROUNDS,
    )
    parser.add_argument("--metric", type=str, default=constants.DEFAULT_METRIC)
    parser.add_argument(
        "--learning_rate", type=float, default=constants.DEFAULT_LEARNING_RATE
    )
    parser.add_argument("--num_leaves", type=int, default=constants.DEFAULT_NUM_LEAVES)
    parser.add_argument(
        "--feature_fraction", type=float, default=constants.DEFAULT_FEATURE_FRACTION
    )
    parser.add_argument(
        "--bagging_fraction", type=float, default=constants.DEFAULT_BAGGING_FRACTION
    )
    parser.add_argument(
        "--bagging_freq", type=int, default=constants.DEFAULT_BAGGING_FREQUENCE
    )
    parser.add_argument("--max_depth", type=int, default=constants.DEFAULT_MAX_DEPTH)
    parser.add_argument(
        "--min_data_in_leaf", type=int, default=constants.DEFAULT_MIN_DATA_IN_LEAF
    )
    parser.add_argument(
        "--max_delta_step", type=float, default=constants.DEFAULT_MAX_DELTA_STEP
    )
    ...
    parser.add_argument(
        "--min_gain_to_split", type=float, default=constants.DEFAULT_MIN_GAIN_TO_SPLIT
    )
    parser.add_argument(
        "--scale_pos_weight", type=float, default=constants.DEFAULT_SCALE_POS_WEIGHT
    )
    parser.add_argument(
        "--tree_learner", type=str, default=constants.DEFAULT_TREE_LEARNER
    )
    parser.add_argument(
        "--feature_fraction_bynode",
        type=float,
        default=constants.DEFAULT_FEATURE_FRACTION_BYNODE,
    )
    parser.add_argument(
        "--is_unbalance", type=str, default=constants.DEFAULT_IS_UNBALANCE
    )
    parser.add_argument("--max_bin", type=int, default=constants.DEFAULT_MAX_BIN)
    parser.add_argument(
        "--num_threads", type=int, default=constants.DEFAULT_NUM_THREADS
    )
...
def run_with_args(args):
    ...
    if not (
        args.use_dask == "True" or len(args.hosts) > 1
    ):  # this corresponds to single instance training
        ...
        params = configure_parameters(
            args=args, problem_type=problem_type, is_for_dask_train=False
        )
        ...
        main_host = args.hosts[
            0
        ]  # choose the first host of the host list as the main host
        ...
        if current_host == main_host:
            with Client(
                constants.SCHEDULER_IP_TEMPLATE.format(ip=scheduler_ip)
            ) as client:
                try:
                    ...
                    if args.metric == constants.AUTO:
                        metric = constants.CLASSIFICATION_PROBLEM_TYPE_METRIC_MAPPING[
                            problem_type
                        ]
                    else:
                        metric = args.metric
                    ...
                    params = configure_parameters(
                        args=args, problem_type=problem_type, is_for_dask_train=True
                    )
                    ...
                    booster = utils.get_pretrained_model(
                    ...
                    if not dask_model.fitted_:
                        raise RuntimeError(
                            "Unexpected error detected. Model is not fitted."
                        )
                    logging.info("Done training")

                    utils.save_model(
                        model=dask_model.booster_, model_dir=args.model_dir
                    )
                    model_info.save_model_info(
                        input_model_untarred_path=constants.INPUT_MODEL_UNTARRED_PATH,
                        model_dir=args.model_dir,
